...
if (isset($_SESSION['email'])) {
    ...
    // }else{
    //     $fetPro = "SELECT * FROM combo_offer WHERE id=$pro_id";
    //     $data_pro_res = mysqli_query($conn, $fetPro);
    //     $res_data = mysqli_fetch_assoc($data_pro_res);
    //     print_r($res_data);
    // }
    $quantity = isset($_GET['quantity']) ? $_GET['quantity'] : 1;
    $sale_price = isset($_GET['sale_price']) ? $_GET['sale_price'] : '';
    ...
    if (mysqli_num_rows($data) == 0) {
        ...
        ";
            ...
            $fetcmb = "SELECT * FROM combo_offer WHERE id='$product_id'";
            $data2cmb = mysqli_query($conn, $fetcmb);
            $product_record = mysqli_fetch_assoc($data2cmb);
            $product_name = $product_record['product_name'];
            $primary_image_url = $product_record['primary_image_url'];
            $product_price = $sale_price;
            $sql3 = "INSERT INTO mycarts (product_id, user_email, product_name, product_quantity, primary_image_url, product_price) VALUES ('$product_id','$user_login_mail','$product_name','$quantity','$primary_image_url','$product_price')";
        ...
        $updateSql = "UPDATE mycarts SET product_quantity='$quantity',product_price='$sale_price' WHERE user_email='$user_login_mail' AND product_id='$product_id'";
